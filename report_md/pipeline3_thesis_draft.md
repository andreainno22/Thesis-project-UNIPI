...
**Minimum detectable object size.** Because every image is resized to 224x224 before feature extraction, the 28x28 patch grid corresponds to one patch every 8 px in the resized input, and after the 3x3 neighborhood pooling each feature vector integrates a 24x24 px region. Build and test share the same resize, so the comparison is consistent and unbiased; the real constraint is loss of detail for small objects. An object must occupy at least 2-3 adjacent patches to produce an anomaly signal distinguishable from noise, i.e. roughly 16-24 px in the 224x224 input. Mapping this back to a 960x1280 source image, two patches correspond to ~69x91 px and a full 3x3 neighborhood to ~103x137 px in the original resolution. The Gaussian smoothing (sigma=4) expands blobs further, lowering the perceptual floor, so a 30-40 px object can still produce a visible 60-70 px blob. Typical obstructions (cart, wheelchair, bin) imaged from 2-3 m are well above this limit; the sub-threshold regime concerns only very distant, partially-cropped, or very small objects (e.g. a small parcel).

**Why the bank includes the glass region.** An alternative design would exclude the glass panel from the build phase, storing only jamb patches in the bank. This is counterproductive: at test time the model still extracts features from the whole image, and for glass-region patches it searches for nearest neighbors in a bank that contains none. The resulting kNN distance is high not because of an anomaly but because of a pure absence of reference - a distribution mismatch. Scores in that region would be systematically inflated even on perfectly normal scenes, making the glass area anomalous by construction. The correct choice is therefore to include the full image in the bank and let the gating filter out, a posteriori, the blobs that do not touch the frame.

**Structural limitations of the gating rule.** The mechanism rests on an implicit assumption: the annotated frame geometry is stable and visible at test time. Two operationally relevant scenarios violate it. First, an **open door** produces a symmetric double failure mode: the threshold and jambs - which supplied normal patches to the bank when closed - change radically (the threshold rotates away with the leaf, the jambs are partially covered), producing an out-of-distribution component exactly inside the frame mask that trips the alarm regardless of obstruction (structural false positive); simultaneously, an obstacle placed in an open doorway no longer produces a component intersecting the (now misplaced) frame mask, so gating rejects a real obstruction (structural false negative). This is the most concerning case for an emergency-exit system, since an exit is often blocked precisely while the door stands open. Second, a **partially occluded jamb** (e.g. a tall object resting against the frame) can hide part of the mask, so the main obstruction's anomaly component may fail to reach the still-visible portion. Both share the same root cause: gating assumes the frame mask is always reachable by a genuine anomaly component, but reachability depends on frame visibility, which is a property of the scene, not of the model. Candidate mitigations - a door-state classifier that disables gating when the door is open, separate references per door state, or runtime-adaptive frame masks - are architectural extensions left as future work. In this thesis the open-door case is declared as a method limitation: the system is designed and validated for the nominal condition (closed door, visible frame), which is the dominant condition in continuous surveillance where the door is open only transiently.
